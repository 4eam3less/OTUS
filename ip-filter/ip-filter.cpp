...
int main() {
    ...
    try {
        ...
        std::for_each(ip_pool.cbegin(), ip_pool.cend(),
                      [](const std::vector<uint8_t> &ip) { if (ip[0] == 1) std::cout << ip; });
        std::for_each(ip_pool.cbegin(), ip_pool.cend(),
                      [](const std::vector<uint8_t> &ip) { if (ip[0] == 46 && ip[1] == 70) std::cout << ip; });
        std::for_each(ip_pool.cbegin(), ip_pool.cend(), [](const std::vector<uint8_t> &ip) {
